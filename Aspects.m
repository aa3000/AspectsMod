...
static SEL aspect_aliasForClassSelector(SEL selector, Class klass) {
    NSCParameterAssert(selector);
    NSCParameterAssert(klass);
    return NSSelectorFromString([AspectsMessagePrefix stringByAppendingFormat:@"_%@_%@", NSStringFromClass(klass), NSStringFromSelector(selector)]);
}
...
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    ...
    if (!aspect_isMsgForwardIMP(targetMethodIMP)) {
        ...
        // modify by Liam, make all class selector get hook
...
static void __ASPECTS_ARE_BEING_CALLED__(__unsafe_unretained NSObject *self, SEL selector, NSInvocation *invocation) {
    ...
    // Add By Liam(1), Fixed Infinite Loop when overriding a method at two points which includes a call to super
    Class realClass = getRealClass(self, object_getClass(self), invocation.selector);
    // end Add By Liam(1)
    ...
    AspectsContainer *classContainer = aspect_getContainerForClass(realClass, aliasSelector);
    ...
    if (objectContainer.insteadAspects.count || classContainer.insteadAspects.count) {
        ...
        // Add By Liam(2), Fixed Infinite Loop when overriding a method at two points which includes a call to super
        if (klass != realClass)
        {
//            id block = ^(){};
//
//            AspectBlockRef pBlockStruct = (__bridge void *)block;
//            pBlockStruct->invoke = class_getMethodImplementation(realClass, aliasSelector);
//            
//            invocation.target = block;
            
            // first solution
            klass = realClass;
            invocation.selector = aspect_aliasForClassSelector(originalSelector, klass);

            if (![klass instancesRespondToSelector:invocation.selector]) {
                Method method = class_getInstanceMethod(klass, aliasSelector);
                class_addMethod(klass, invocation.selector, method_getImplementation(method), method_getTypeEncoding(method));
            }
        }
        // end Add By Liam(2)
